...
SWEEP_LOOKBACK_BARS = 7    # how many bars back a sweep can have happened
                            # and still "count" as confirmation for a signal
...
def compute_liquidity_sweeps(df: pd.DataFrame, piv_high: pd.Series, piv_low: pd.Series):
    """
    Buy-side sweep (bearish precursor): current high wicks ABOVE the most
    recent confirmed pivot high, but candle CLOSES back below it.
    Sell-side sweep (bullish precursor): current low wicks BELOW the most
    recent confirmed pivot low, but candle CLOSES back above it.

    Returns two boolean Series aligned to df.index.
    """
    n = len(df)
    buy_side_sweep = pd.Series(False, index=df.index)   # bearish signal precursor
    sell_side_sweep = pd.Series(False, index=df.index)  # bullish signal precursor

    last_piv_high_val = None
    last_piv_low_val = None

    for i in range(n):
        # Update "known" pivots (only known lookback bars after they form)
        if piv_high.iloc[i]:
            last_piv_high_val = df["high"].iloc[i]
        if piv_low.iloc[i]:
            last_piv_low_val = df["low"].iloc[i]

        if last_piv_high_val is not None:
            if df["high"].iloc[i] > last_piv_high_val and df["close"].iloc[i] < last_piv_high_val:
                buy_side_sweep.iloc[i] = True
                last_piv_high_val = None  # consumed, wait for a new pivot

        if last_piv_low_val is not None:
            if df["low"].iloc[i] < last_piv_low_val and df["close"].iloc[i] > last_piv_low_val:
                sell_side_sweep.iloc[i] = True
                last_piv_low_val = None  # consumed

    return buy_side_sweep, sell_side_sweep


def sweep_happened_recently(sweep_series: pd.Series, idx: int, bars: int) -> bool:
    start = max(0, idx - bars)
    return bool(sweep_series.iloc[start: idx + 1].any())
...
def evaluate_symbol(df: pd.DataFrame):
    ...
    buy_side_sweep, sell_side_sweep = compute_liquidity_sweeps(df, piv_high, piv_low)
    df["buy_side_sweep"] = buy_side_sweep     # bearish precursor
    df["sell_side_sweep"] = sell_side_sweep   # bullish precursor
    ...
    rsi_signal_bull = row["rsi_os"] or prev["rsi_os"]
    rsi_signal_bear = row["rsi_ob"] or prev["rsi_ob"]

    buy_trigger = rsi_signal_bull and row["bull_engulf"]
    sell_trigger = rsi_signal_bear and row["bear_engulf"]

    liquidity_confirms_buy = sweep_happened_recently(closed["sell_side_sweep"], i, SWEEP_LOOKBACK_BARS)
    liquidity_confirms_sell = sweep_happened_recently(closed["buy_side_sweep"], i, SWEEP_LOOKBACK_BARS)
    ...
    if buy_trigger and liquidity_confirms_buy:
        return "BUY", info
    if sell_trigger and liquidity_confirms_sell:
        return "SELL", info
                ...
                if signal:
                    ...
                    if candle_key not in alerted:
                        msg = (
                            ...
                            f"Confirmed by liquidity sweep within last {SWEEP_LOOKBACK_BARS} candles"
                        )
                        send_telegram(msg)
