...
def evaluate_symbol(df: pd.DataFrame):
    ...
    # Candle B must engulf Candle A's BODY -- regardless of which color
    # Candle A closed. This is a color-agnostic engulfing check: Candle A's
    # own color doesn't matter (it can be red or green), only that its wick
    # swept liquidity and its close broke structure. What matters for
    # Candle B is that its body fully covers Candle A's body.
    prev_body_top = max(prev["open"], prev["close"])
    prev_body_bottom = min(prev["open"], prev["close"])
    engulfs_prev_bull = row["close"] > prev_body_top and row["open"] < prev_body_bottom
    engulfs_prev_bear = row["close"] < prev_body_bottom and row["open"] > prev_body_top

    # Candle A must be the sweep+BOS candle; Candle B (current) must engulf it
    buy_trigger = prev["bull_sweep_bos"] and engulfs_prev_bull and rsi_signal_bull
    sell_trigger = prev["bear_sweep_bos"] and engulfs_prev_bear and rsi_signal_bear
...
def debug_funnel_counts(df: pd.DataFrame) -> dict:
    ...
    bull_engulf_relaxed = 0
    bear_engulf_relaxed = 0

    for i in range(2, n):
        prev_o, prev_c = df["open"].iloc[i - 1], df["close"].iloc[i - 1]
        body_top = max(prev_o, prev_c)
        body_bottom = min(prev_o, prev_c)
        row_o, row_c = df["open"].iloc[i], df["close"].iloc[i]

        engulfs_bull = row_c > body_top and row_o < body_bottom
        engulfs_bear = row_c < body_bottom and row_o > body_top
        if engulfs_bull:
            bull_engulf_relaxed += 1
        if engulfs_bear:
            bear_engulf_relaxed += 1

        if bull_sweep_bos.iloc[i - 1] and engulfs_bull:
            bull_full += 1
            if df["rsi_os"].iloc[i - 2] or df["rsi_os"].iloc[i - 1]:
                bull_full_with_rsi += 1
        if bear_sweep_bos.iloc[i - 1] and engulfs_bear:
            bear_full += 1
